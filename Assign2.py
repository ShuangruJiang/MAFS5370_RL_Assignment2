...
@torch.no_grad()
def evaluate_random_baseline_stats(episodes: int = 200, seed: Optional[int] = None) -> dict:
    """Uniform-random legal moves vs the environment's random opponent (same as training opponent)."""
    if seed is not None:
        random.seed(seed)
    env = BoardGameEnv()
    wins = losses = draws = 0
...
def print_vs_random_experiment_report(title: str, stats: dict) -> None:
    n = stats["wins"] + stats["losses"] + stats["draws"]
    print(f"\n--- {title} ---")
    print(f"Games: {n}")
    print(f"Wins / Losses / Draws: {stats['wins']} / {stats['losses']} / {stats['draws']}")
...
def experiment_vs_random_opponent(
    ...
    """
    Run evaluation games: greedy Q-policy vs env random opponent, optionally vs uniform-random baseline.
    Returns (learned_stats, baseline_stats_or_None).
    """
    device = device or ("cuda" if torch.cuda.is_available() else "cpu")
    learned = evaluate_stats(policy_net, episodes=episodes, device=device)
    print_vs_random_experiment_report("Greedy policy vs random opponent", learned)

    baseline_stats = None
    if baseline_episodes is not None and baseline_episodes > 0:
        baseline_stats = evaluate_random_baseline_stats(episodes=baseline_episodes, seed=baseline_seed)
        print_vs_random_experiment_report("Uniform-random actions vs random opponent (baseline)", baseline_stats)

    return learned, baseline_stats
...
if __name__ == "__main__":
    log_path = f"assign2_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    _original_stdout = sys.stdout
    _log_fp = open(log_path, "w", encoding="utf-8")
    sys.stdout = _TeeStdout(_log_fp, _original_stdout)

    try:
        print(f"Full transcript (this run) is also saved to: {log_path}\n")
        ...
        print("\n========== Experiment: vs random opponent ==========")
        experiment_vs_random_opponent(
            q_net,
            episodes=config.eval_episodes,
            device=config.device,
            baseline_episodes=config.eval_episodes,
        ...
        _log_fp.close()
        print(f"Transcript saved to: {log_path}", file=_original_stdout)
